...
async function createAppleJwt(config) {
	const privateKey = await (0, jose.importPKCS8)(config.privateKey, ALGORITHM);
	const now = Math.floor(Date.now() / 1e3);
	return new jose.SignJWT({}).setProtectedHeader({
		alg: ALGORITHM,
		kid: config.keyId,
		typ: "JWT"
...
//#endregion
exports.AppleAppStoreAdapter = AppleAppStoreAdapter;
exports.AppleAppStoreError = AppleAppStoreError;
exports.createAppleJwt = createAppleJwt;
